...
class WXR_Importer extends WP_Importer {
	...
	/**
	 * Map of remote attachment URL => locally downloaded temp file path,
	 * populated by a concurrent prefetch pass before the main import loop.
	 *
	 * @since 2.1.0
	 * @var string[]
	 */
	protected $prefetched_files = array();
	...
	/**
	 * Importer options, as passed to the constructor.
	 *
	 * @since 2.1.0
	 * @var array
	 */
	...
	public function set_logger( WP_Importer_Logger $logger ): void {
		$this->logger = $logger;
	}
	 ...
	 */
	protected function get_reader( string $file ): XMLReader|WP_Error {
		$reader = new XMLReader();

		// Avoid loading external entities/DTDs and disallow network access
		// during parsing (XXE hardening). As of libxml 2.9.0 (bundled with
		// PHP 8.0+), external entity loading is already disabled by
		// default, but this parses user-uploaded XML, so it's set
		// explicitly here as defense in depth rather than relying on the
		// libxml default.
	 ...
	 */
	public function get_preliminary_information( string $file ): WXR_Import_Info|WP_Error {
	 ...
	 * @param string $file Path to the WXR file for importing
	 */
	public function parse_authors( string $file ): array|WP_Error {
		// Let's run the actual importer now, woot
		$reader = $this->get_reader( $file );
		if ( is_wp_error( $reader ) ) {
			return $reader;
	 ...
	 * @param string $file Path to the WXR file for importing
	 */
	public function import( string $file ): ?WP_Error {
		...
		// If we're fetching attachments, download them all concurrently
		// up front rather than one at a time as each post is encountered.
		// This only front-loads network I/O — it doesn't change import
		// order, deduplication, or any other behavior of the main loop
		// below, which is unmodified. A prefetch failure for a given URL
		// just means that attachment falls back to the normal synchronous
		// fetch later; it's never fatal to the import.
		if ( $this->options['fetch_attachments'] && apply_filters( 'wxr_importer.prefetch_attachments', true ) ) {
			$urls = $this->find_attachment_urls_for_prefetch( $file );
			if ( ! empty( $urls ) ) {
				$this->logger->info( sprintf(
					/* translators: %d: number of attachment files */
					__( 'Prefetching %d attachment(s)…', 'wordpress-importer' ),
					count( $urls )
				) );
				$this->prefetch_remote_files( $urls );
			}
		}
	 ...
	 */
	protected function log_error( WP_Error $error ): void {
	 ...
	 * @param string $file Path to the WXR file for importing
	 */
	protected function import_start( string $file ): ?WP_Error {
		if ( ! is_file( $file ) ) {
		...
		// Prefill exists calls if told to. Above a size threshold, skip
		// prefilling by default — loading every existing post/comment/term
		// into memory up front doesn't scale, and a per-item DB lookup
		// becomes the better trade once there's enough data that the
		// prefill itself is the memory risk. Duplicates are still caught
		// either way; only the lookup strategy changes. Disable this
		// auto-behavior entirely via `wxr_importer.auto_disable_prefill`,
		// or tune the threshold via `wxr_importer.prefill_threshold_bytes`.
		$large_import = false;
		if ( apply_filters( 'wxr_importer.auto_disable_prefill', true ) ) {
			$threshold = (int) apply_filters( 'wxr_importer.prefill_threshold_bytes', 20 * MB_IN_BYTES );
			$filesize = @filesize( $file );
			$large_import = ( false !== $filesize && $filesize > $threshold );

			if ( $large_import ) {
				$this->logger->notice( sprintf(
					__( 'Import file is %s; skipping upfront duplicate-checking prefill to limit memory use. Duplicates are still detected, just checked one at a time instead of all at once.', 'wordpress-importer' ),
					size_format( $filesize )
				) );
			}
		}

		if ( $this->options['prefill_existing_posts'] && ! $large_import ) {
			$this->prefill_existing_posts();
		}
		if ( $this->options['prefill_existing_comments'] && ! $large_import ) {
			$this->prefill_existing_comments();
		}
		if ( $this->options['prefill_existing_terms'] && ! $large_import ) {
	 ...
	 */
	protected function import_end(): void {
	 ...
	 */
	public function set_user_mapping( array $mapping ): void {
	 ...
	 */
	public function set_user_slug_overrides( array $overrides ): void {
	 ...
	 */
	protected function parse_post_node( DOMElement $node ): array|WP_Error {
		$data = array();
		$meta = array();
		$comments = array();
		$terms = array();
	 ...
	 */
	protected function process_post( array $data, array $meta, array $comments, array $terms ): ?bool {
		/**
	 ...
	 */
	protected function process_menu_item_meta( int $post_id, array $data, array $meta ): void {
	 ...
	 */
	protected function process_attachment( array $post, array $meta, string $remote_url ): int|WP_Error {
	 ...
	 */
	protected function parse_meta_node( DOMElement $node ): ?array {
		foreach ( $node->childNodes as $child ) {
			// We only care about child elements
			if ( $child->nodeType !== XML_ELEMENT_NODE ) {
				continue;
	 ...
	 * @return bool False if a filtered meta item was empty, true otherwise.
	 */
	protected function process_post_meta( array $meta, int $post_id, array $post ): bool {
		if ( empty( $meta ) ) {
			return true;
		}
	 ...
	 */
	protected function parse_comment_node( DOMElement $node ): array {
		$data = array(
			'commentmeta' => array(),
		);
	 ...
	 * @param int $post_id Post to associate with.
	 * @param array $post Post data.
	 * @return int|false Number of comments imported on success, false if a filtered comment was empty.
	 */
	protected function process_comments( array $comments, int $post_id, array $post, int|false $post_exists = false ): int|false {
	...
	protected function parse_category_node( DOMElement $node ): ?array {
		$data = array(
	 ...
	 * @return int
	 */
	public static function sort_comments_by_id( array $a, array $b ): int {
	...
	protected function parse_author_node( DOMElement $node ): array {
		$data = array();
		$meta = array();
		foreach ( $node->childNodes as $child ) {
			// We only care about child elements
	...
	protected function process_author( array $data, array $meta ): ?bool {
		/**
	...
	protected function parse_term_node( DOMElement $node, string $type = 'term' ): ?array {
		$data = array();
		$meta = array();
	...
	protected function process_term( array $data, array $meta ): ?bool {
		/**
	...
	/**
	 * Do a lightweight pass over the WXR file to collect attachment URLs,
	 * for the concurrent prefetch pass — separate from (and much cheaper
	 * than) the full parse the main import loop does.
	 *
	 * @since 2.1.0
	 *
	 * @param string $file Path to the WXR file.
	 * @return string[] Unique attachment URLs found.
	 */
	protected function find_attachment_urls_for_prefetch( string $file ): array {
		$reader = $this->get_reader( $file );
		if ( is_wp_error( $reader ) ) {
			return array();
		}
		...
		$urls = array();
		while ( $reader->read() ) {
			if ( $reader->nodeType === XMLReader::ELEMENT && $reader->name === 'wp:attachment_url' ) {
	...
	/**
	 * Download a batch of remote files concurrently via curl_multi, ahead
	 * of the main import loop reaching them one at a time.
	 *
	 * Purely a performance optimization: populates $this->prefetched_files
	 * for fetch_remote_file() to consume. Never fatal — any URL that fails
	 * to prefetch (including on hosts without the curl extension) is
	 * simply left for fetch_remote_file() to fetch synchronously as usual.
	 *
	 * @since 2.1.0
	 *
	 * @param string[] $urls URLs to prefetch.
	 */
	protected function prefetch_remote_files( array $urls ): void {
		if ( ! function_exists( 'curl_multi_init' ) ) {
			return;
		}

		$concurrency = max( 1, (int) apply_filters( 'wxr_importer.prefetch_concurrency', 4 ) );
		$queue = $urls;
		$multi = curl_multi_init();
		$active = array(); // handle id => array( 'url' => ..., 'handle' => ..., 'tmp_file' => ..., 'fh' => ... )

		$start_next = function () use ( &$queue, &$active, $multi ) {
			...
			curl_setopt( $ch, CURLOPT_USERAGENT, 'WordPress/Importer;' . home_url() );

			curl_multi_add_handle( $multi, $ch );
			$active[ (int) $ch ] = array( 'url' => $url, 'handle' => $ch, 'tmp_file' => $tmp_file, 'fh' => $fh );
		};
		...
		do {
			...
			while ( $info = curl_multi_info_read( $multi ) ) {
				...
				$code = (int) curl_getinfo( $entry['handle'], CURLINFO_HTTP_CODE );
				if ( CURLE_OK === $info['result'] && 200 === $code && filesize( $entry['tmp_file'] ) > 0 ) {
					$this->prefetched_files[ $entry['url'] ] = $entry['tmp_file'];
				} else {
				...
				$start_next();
				$running = 1; // There may be more queued work even if curl briefly reports none active.
			}
		} while ( $running > 0 || ! empty( $queue ) || ! empty( $active ) );
	 ...
	 */
	protected function fetch_remote_file( string $url, array $post ): array|WP_Error {
		...
		if ( isset( $this->prefetched_files[ $url ] ) ) {
			$prefetched = $this->prefetched_files[ $url ];
			unset( $this->prefetched_files[ $url ] );

			if ( ! copy( $prefetched, $upload['file'] ) ) {
				@unlink( $prefetched );
				return new WP_Error( 'import_file_error', __( 'Could not use the prefetched file.', 'wordpress-importer' ) );
			}
			@unlink( $prefetched );

			return $this->validate_fetched_file( $upload );
		}
	...
	/**
	 * Validate a downloaded file's size against zero-size and max-size
	 * limits — the checks shared between the synchronous and prefetched
	 * download paths in fetch_remote_file().
	 *
	 * @since 2.1.0
	 *
	 * @param array $upload Return value of wp_upload_bits().
	 * @return array|WP_Error $upload unchanged on success, WP_Error otherwise.
	 */
	protected function validate_fetched_file( array $upload ): array|WP_Error {
		$filesize = filesize( $upload['file'] );
	...
	protected function post_process(): void {
	...
	protected function post_process_posts( array $todo ): void {
	...
	protected function post_process_menu_item( int $post_id ): void {
	...
	protected function post_process_comments( array $todo ): void {
	 ...
	 */
	protected function replace_attachment_urls_in_content(): void {
		global $wpdb;
	 ...
	 */
	function remap_featured_images(): void {
	 ...
	 */
	public function is_valid_meta_key( string $key ): string|false {
	 ...
	 * @return int 60
	 */
	// Signatures below intentionally untyped: both override WP_Importer
	// (wp-admin/includes/class-wp-importer.php), whose own declarations
	// have no types — PHP requires an override to stay compatible with
	// the parent's signature, so adding types here would be a fatal
	 ...
	 */
	protected function prefill_existing_posts(): void {
		global $wpdb;
	 ...
	 */
	protected function post_exists( array $data ): int|false {
	 ...
	 */
	protected function mark_post_exists( array $data, int $post_id ): void {
		$exists_key = $data['guid'];
	 ...
	 */
	protected function prefill_existing_comments(): void {
		global $wpdb;
	 ...
	 */
	protected function comment_exists( array $data ) {
	 ...
	 */
	protected function mark_comment_exists( array $data, $comment_id ): void {
		$exists_key = sha1( $data['comment_author'] . ':' . $data['comment_date'] );
	 ...
	 * @see self::prefill_existing_posts() for justification of why this exists.
	 */
	protected function prefill_existing_terms(): void {
		global $wpdb;
	 ...
	 */
	protected function mark_term_exists( array $data, $term_id ): void {
		$exists_key = sha1( $data['taxonomy'] . ':' . $data['slug'] );
